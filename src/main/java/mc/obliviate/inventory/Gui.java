...
public abstract class Gui implements InventoryHolder {
	...
	private final Map<Integer, mc.obliviate.inventory.GuiIcon> registeredIcons;
//	private final List<MyScheduledTask> taskList = new ArrayList<>();
	...
	public Gui(@Nonnull Player player, @Nonnull String id, String title, @Nonnegative int rows) {
		this(player, id, LegacyComponentSerializer.legacySection().deserialize(title), rows);
	}

	public Gui(@Nonnull Player player, @Nonnull String id, String title, InventoryType inventoryType) {
		this(player, id, LegacyComponentSerializer.legacySection().deserialize(title), inventoryType);
	}
	...
	public Plugin getPlugin() {
		return mc.obliviate.inventory.InventoryAPI.getInstance().getPlugin();
	}
	...
	public void onClose(InventoryCloseEvent event) {
		...
		final Gui gui = mc.obliviate.inventory.InventoryAPI.getInstance().getGuiFromInventory(event.getPlayer().getOpenInventory().getTopInventory());
		if (gui == null) return;
...
//		gui.stopAllTasks();
	 ...
	 */
	public void open() {
		Preconditions.checkNotNull(mc.obliviate.inventory.InventoryAPI.getInstance(), "Inventory API is not initialized. Please use new InventoryAPI().init() also, you can visit wiki of obliviate-invs.");
		final Gui gui = mc.obliviate.inventory.InventoryAPI.getInstance().getPlayersCurrentGui(this.player);
		if (gui != null) {
		...
		mc.obliviate.inventory.InventoryAPI.getInstance().getPlayers().put(this.player.getUniqueId(), this);
	 ...
	 */
	public void fillGui(mc.obliviate.inventory.GuiIcon icon) {
	...
	public void fillGui(ItemStack item) {
		this.fillGui(new mc.obliviate.inventory.Icon(item));
	}
	...
	public void fillGui(Material material) {
		this.fillGui(new mc.obliviate.inventory.Icon(material));
	}
	 ...
	 */
	public void fillGui(mc.obliviate.inventory.GuiIcon icon, Iterable<Integer> blacklisted_slots) {
		for (int slot = 0; slot < size; slot++) {
	 ...
	 */
	public void fillRow(mc.obliviate.inventory.GuiIcon item, @Nonnegative int row) {
	 ...
	 */
	public void fillColumn(mc.obliviate.inventory.GuiIcon item, @Nonnegative int column) {
	...
	public void addItem(@Nonnegative int slot, @Nullable mc.obliviate.inventory.GuiIcon icon) {
	...
	public void addItem(@Nullable mc.obliviate.inventory.GuiIcon item, @Nonnull Integer... slots) {
		for (int slot : slots) {
			this.addItem(slot, item);
		}
	}

	public void addItem(@Nullable mc.obliviate.inventory.GuiIcon icon, @Nonnull Iterable<Integer> slots) {
	...
	public void addItem(@Nonnegative int slot, @Nullable ItemStack item) {
		this.addItem(slot, new mc.obliviate.inventory.Icon(item));
	}

	public void addItem(@Nonnull mc.obliviate.inventory.GuiIcon icon) {
		this.addItem(this.inventory.firstEmpty(), icon);
	}

	public void addItem(@Nullable ItemStack item) {
		this.addItem(this.inventory.firstEmpty(), new mc.obliviate.inventory.Icon(item));
	}

	public void addItem(@Nonnull Material material) {
		this.addItem(this.inventory.firstEmpty(), new mc.obliviate.inventory.Icon(material));
	}
...
//		Preconditions.checkNotNull(mc.obliviate.mc.obliviate.inventory.InventoryAPI.getInstance(), "InventoryAPI is not initialized.");
//		final MyScheduledTask[] scheduledTask = new MyScheduledTask[]{null};
//		scheduledTask[0] = mc.obliviate.mc.obliviate.inventory.InventoryAPI.getScheduler().runTaskTimer(() -> update.accept(scheduledTask[0]), runDelayInTicks, periodInTicks);
